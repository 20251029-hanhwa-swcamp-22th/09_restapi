...
*   (API진화에 유연하게 대응할 수 있다)
*
* - REST API 성숙도 모델 (0 ~ 3 레벨) 중 3레벨을 달성하기 위한 기술
* LV.0 : REST원칙을 지키지 않은 상태
* LV.1 : 자원 개념만 도입
* LV.2 : HTTP 메서드를 황용 (GET, POST, PUT, DELETE, ...) + HTTP 상태코드 사용 (200, 404, 500, ...)
* LV.3 : 응답 본문에 다음 수행할 수 있는 작업 링크를 포함
* LV.2 : HTTP 메서드를 황용 (GET, POST, PUT, DELETE)
*
* */

// API가 JSON을 반환할 때 hateoas를 구현하기가 어려워
@RestController
...
public class HateoasTestController {
	...
	public ResponseEntity<ResponseMessage> findAllUsers() {
        ...
        /* EntityModel : 데이터와 링크를 함 께 담는 래퍼 클래스
        *       -> Spring HATEOAS 핵심 클래스*/
		List<EntityModel<UserDTO>> userWithRel =
				users.stream().map( // stream의 map은 새스트림 생성
					user -> EntityModel.of(
						user,
						linkTo(
							methodOn(HateoasTestController.class)
							.findUserByNo(user.getNo())
						)// http://lacalhost:8080/hateoas/users/{userNo}
						.withSelfRel(),
						linkTo(
							methodOn(HateoasTestController.class)
							.findAllUsers()
						) // http://lacalhost:8080/hateoas/users
