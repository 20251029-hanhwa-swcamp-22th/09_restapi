...
@Service
@RequiredArgsConstructor // final이 붙은 필드를 초기화하는 생성자 추가하는 어노테이션
public class ProductQueryService {
    ...
    public ProductDetailResponse getProduct(Long productCode){
        /* Mybatis는 매퍼를 톨해서 조회 결과를 바로 DTO에 매핑할 수 있음
        * -> JPA 보단 편함 (Query Side 장점)*/
        ...
        ProductDTO product
                = Optional.ofNullable(
                        // mybatis를 이용해 db에서 확인
                productMapper.selectProductByCode(productCode)

                // 상수열거형인 에러코드에서 PRODUCT_NOT_FOUND를 꺼네오겠다
        ...
        return ProductListResponse.builder()
                ...
                .pagination(
                        Pagination.builder()
                                ...
                                .totalPage((int)Math.ceil((double) totalItem / size))// ex) 21/10 -> 2.1  Math.ceil -> 3
