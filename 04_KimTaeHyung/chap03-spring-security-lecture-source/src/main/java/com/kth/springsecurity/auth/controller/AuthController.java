package com.kth.springsecurity.auth.controller;

import com.kth.springsecurity.auth.dto.LoginRequest;
import com.kth.springsecurity.auth.dto.TokenResponse;
import com.kth.springsecurity.auth.service.AuthService;
import com.kth.springsecurity.common.ApiResponse;
  ...
  * */
  ...
  private ResponseCookie createDeleteRefreshTokenCookie() {
    return ResponseCookie.from("refreshToken", "")
        ...
        // .secure(true)                    // HTTPS 환경일 때만 전송 (운영 환경에서 활성화 권장)
        .path("/")                          // 쿠키 범위 : 전체 경로
        .maxAge(Duration.ofDays(0))         // 쿠키 만료 기간 : 0 -> 즉시 만료
        .sameSite("Strict")                 // CSRF 공격 방어를 위한 SameSite 설정
        .build();
  }
